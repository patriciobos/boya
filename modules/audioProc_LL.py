...
_candidate = (
    BASE_DIR / "test" / "test_recordings" / "20180824_8105_20m_daspre_cap_2.wav"
)
...
class AudioProcLowLevel:
    ...
    def init(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                "AudioProc module initialized: test_wav_path=%s", self.test_wav_path
            )
            return True
    ...
    def lpf_butterworth(self, wav_path, cutoff_hz=200, order=6, output_path=None):
        ...
        self.logger.info(
            f"Applying LPF Butterworth: {wav_path}, cutoff={cutoff_hz} Hz, order={order}"
        )
        # Read WAV file
        ...
        if not (0 < normal_cutoff < 1):
            raise ValueError(
                f"Cutoff frequency too high for sampling rate: normal_cutoff={normal_cutoff}. Must be between 0 and 1."
            )
        ba = butter(order, normal_cutoff, btype="low", analog=False)
        if ba is None or not isinstance(ba, (tuple, list)) or len(ba) != 2:
            raise RuntimeError(
                "Failed to design Butterworth filter: butter() returned None or invalid output."
            )
        b, a = ba
    ...
    def BL_calculator(self, wav_path):
        ...
        if n_channels == 1:
            ...
            L = len(
                audio[:, 0]
            )  # Length of first channel (all channels have same length)
        ...
        if L < min_length_factor * nperseg:
            ...
            noverlap = nperseg // 2
            self.logger.info(
                f"Adjusted nperseg to {nperseg} and noverlap to {noverlap} "
        ...
        f_max = 100000.0
        bands_df = bands_df[
            (bands_df["fl"] >= f_min) & (bands_df["fh"] <= f_max)
        ].copy()
        ...
        self.logger.info(
            f"Calculation completed. {n_bands_valid} bands processed using Welch estimator for PSD."
        )
        return powers_dB
    ...
    def rel_band_power_calculator(self, powers):
        ...
        """
        self.logger.info(
            "Calculating relative power with respect to minimum reference (noise_ref from config)..."
        )
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load configs/config.json at '{config_path}': {e}"
            )
        ...
        if {"freq", "psd"}.issubset(ref_df.columns):
            ...
            if ref_df.shape[1] < 2:
                raise ValueError(
                    f"noise_ref file '{noise_ref_path}' must have at least two columns (freq, psd)"
                )
        ...
        # Avoid log of zero
        ref_powers_safe = np.where(
            np.isfinite(ref_powers), np.maximum(ref_powers, 1e-20), ref_powers
        )
        ...
        if n_bands_meas != len(reference_minimum_dB):
            raise ValueError(
                f"Number of bands in measured powers ({n_bands_meas}) does not match number of reference bands ({len(reference_minimum_dB)})"
            )
    ...
    def _output_timestamp_from_wav(self, wav_path):
        ...
        ).replace(tzinfo=now_utc_minus_3().tzinfo)
    ...
    def generate_json_output(self, rel_powers, wav_path):
        ...
        output_dir = (
            ...
            else Path(base_dir) / "data" / "audio_proc"
        )
    ...
    def process(self, wav_path):
        ...
        try:
            ...
            self.logger.info(
                "Step 2: Calculating relative powers with respect to reference..."
            )
            ...
            self.logger.info(
                f"Processing completed successfully. Generated file: {output_path}"
            )
            return output_path
    ...
    def full_test(self) -> tuple[bool, dict]:
        ...
        if self.test_wav_path is None:
            error_msg = (
                "test_wav_path is not defined. Must set it before running full_test."
            )
        ...
        try:
            ...
            if is_in_test_recordings:
                ...
                expected_payload = json.loads(
                    Path(expected_json_path).read_text(encoding="utf-8")
                )
                actual = np.asarray(
                    actual_payload["relative_band_power_db"], dtype=float
                )
                expected = np.asarray(
                    expected_payload["relative_band_power_db"], dtype=float
                )
                ...
                if not np.allclose(
                    actual, expected, rtol=0.0, atol=0.001, equal_nan=True
                ):
