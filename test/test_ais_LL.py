...
def test_ais_ll_device_present_and_fix():
    ...
    ll = AISLowLevel(
        preferred_port=preferred, scan_window=scan_window, wait_for_fix=wait_for_fix
    )
    detected = ll.init(scan_window=max(6.0, wait_for_fix))
    assert (
        detected
    ), "No NMEA device detected on serial ports (check PREFERRED_PORT or connect device)"
