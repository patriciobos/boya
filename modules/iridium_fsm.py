...
from modules.support import iridium_payloads
...
from modules.support.iridium_payloads import (
    ...
    load_audio_proc_data,
    load_latest_audioproc_output,
    resolve_audio_output_path,
)
from modules.support.system_config import PROJECT_ROOT, get_config_value, get_data_path, get_logs_path, utc_minus_3_timestamp
...
class IridiumHandlerFSM(BaseHandlerFSM):
    ...
    def _sync_payload_helper_config(self):
        iridium_payloads.PROJECT_ROOT = PROJECT_ROOT
        iridium_payloads.get_config_value = get_config_value
        iridium_payloads.get_data_path = get_data_path
        iridium_payloads.get_logs_path = get_logs_path

    def _build_system_status_payload(self):
        self._sync_payload_helper_config()
        return build_current_system_status_payload()

    def _resolve_audio_output_path(self, output_path):
        self._sync_payload_helper_config()
        return resolve_audio_output_path(output_path)

    def _load_latest_audio_output(self):
        self._sync_payload_helper_config()
        return load_latest_audioproc_output()

    def _load_audio_proc_data(self, audio):
        self._sync_payload_helper_config()
        return load_audio_proc_data(audio)

    def _build_audio_proc_payload(self, audio):
        self._sync_payload_helper_config()
        return build_latest_audio_proc_payload(audio)
