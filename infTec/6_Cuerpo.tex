...
\begin{itemize}
    ...
    \item \textbf{Sensado meteorológico:} incluye un anemómetro ultrasónico WindSonic, comunicado por puerto serie, destinado al registro de velocidad y dirección del viento. Esta información es relevante para correlacionar las mediciones acústicas con condiciones de superficie, dado que el viento y el oleaje influyen directamente sobre el ruido ambiente submarino.

    \item \textbf{Supervisión energética:} incluye el controlador asociado al sistema de generación y almacenamiento de energía, comunicado mediante puerto serie con protocolo Modbus. Este bloque permite relevar variables como tensión de panel, corriente de panel, corriente de carga, tensión de batería, estado de carga y otros parámetros eléctricos necesarios para la gestión operativa de la boya.

    \item \textbf{Adquisición acústica:} incluye una placa de audio USB Behringer UMC202HD, preamplificadores para hidrófonos DAS-1 y DAS-2 diseñados \textit{ad-hoc}, \citep{AS0518_Preamp}. Este bloque constituye la etapa de adquisición de señales acústicas de la boya que es la finalidad principal de la boya intrumental EAMMRA y fue analizado especialmente para caracterizar sus componentes.

    \item \textbf{Telemetría satelital:} incluye un módem Iridium EDGE, compatible con el servicio SBD. Se utiliza para transmitir mensajes binarios de estado y resultados seleccionados de procesamiento. Este subsistema permite supervisión remota de la boya y envío periódico de información crítica.
...
\begin{itemize}
    ...
    \item \texttt{configs/scheduler.json}, que define los intervalos de ejecución del scheduler central para cada módulo.
...
Las pruebas se ejecutan mediante \texttt{pytest}, con separación entre pruebas que no requieren hardware y pruebas orientadas a ejecución sobre periféricos reales. Además, el repositorio incluye scripts de soporte para ejecución de pruebas, cobertura, ensayos de bajo nivel y generación de reportes. Esta organización permite mantener trazabilidad entre código, configuración, pruebas y resultados operacionales.
...
La arquitectura general del sistema se basa en una organización modular orientada a eventos. Cada subsistema se representa mediante una FSM que encapsula sus estados, transiciones, acciones y condiciones de error. La comunicación entre módulos se realiza mediante mensajes, y el Router actúa como intermediario encargado de distribuir eventos entre productores y consumidores.
...
\begin{itemize}
    ...
    \item \textbf{Capa de telemetría:} prepara y transmite mensajes binarios mediante Iridium SBD, incluyendo estados de sistema y resultados seleccionados de procesamiento acústico.
\end{itemize}

El flujo general de operación comienza con la inicialización de configuraciones, módulos y recursos del sistema. Una vez iniciado el proceso principal, el scheduler central dispara eventos de adquisición de acuerdo con intervalos definidos. Los sensores ambientales, meteorológicos, inerciales, de posición y energía generan registros periódicos. La adquisición acústica se ejecuta con una periodicidad menor, debido al mayor volumen de datos generado y a las restricciones de almacenamiento y procesamiento.
