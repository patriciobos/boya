                    ...
                    if isinstance(msg, tuple) and isinstance(msg[1], Message):
                        ...
                        if message.id == MessageID.STATE_CHANGED:
                            ...
                            if "file" in message.params:
                                ...
                                # If a recording was generated by Behringer, route to AudioProc
                                if message.params.get('file') and name == 'Behringer' and 'AudioProc' in fsms:
                                    file_path = message.params.get('file')
                                    logger.info(f"Routing recording {file_path} from {name} to AudioProc for processing")
                                    fsms['AudioProc']['queue'].put(Message(MessageID.SIG_PROCESS, {"file": file_path, "origin": name}))

                        elif message.id == MessageID.RECORDING_DONE:
                            # Dedicated recording notification (preferred)
                            file_path = message.params.get('file')
                            if file_path and 'AudioProc' in fsms:
                                logger.info(f"[router] Received RECORDING_DONE from {name}: {file_path} -> forwarding to AudioProc")
                                fsms['AudioProc']['queue'].put(Message(MessageID.SIG_PROCESS, {"file": file_path, "origin": name}))
